  ...
  deploy-review:
    ...
    steps:
      ...
      - name: Index repository code
        ...
        run: |
          : > /tmp/file_entries.jsonl
          git ls-files | grep -E '\\.(ts|tsx|js|jsx|py|go|rs|java|rb|yml|yaml)$' | while read -r file; do
            jq -n --arg path "$file" --rawfile content "$file" '{path: $path, content: ($content | .[0:5000])}' >> /tmp/file_entries.jsonl
          done

          if [ -s /tmp/file_entries.jsonl ]; then
            jq -s '.' /tmp/file_entries.jsonl > /tmp/files.json
          else
            echo '[]' > /tmp/files.json
          fi

          jq -n --arg repo "$REPO_FULL_NAME" --slurpfile files /tmp/files.json '{repoFullName: $repo, files: $files[0]}' > /tmp/index-payload.json

          set +e
          RESPONSE=$(curl -sS -w "\\n%{http_code}" \\
            -X POST "\${DEPLOYIQ_API_URL}/api/index-repo" \\
            -H "Content-Type: application/json" \\
            -H "Authorization: Bearer \${DEPLOYIQ_API_KEY}" \\
            -d @/tmp/index-payload.json)
          CURL_EXIT=$?
          set -e

          if [ "$CURL_EXIT" -ne 0 ]; then
            echo "::warning title=DeployIQ Indexing Failed::curl exited with status \${CURL_EXIT}"
          else
            HTTP_CODE=$(echo "$RESPONSE" | tail -1)
            BODY=$(echo "$RESPONSE" | sed '$d')

            if [ "$HTTP_CODE" != "200" ]; then
              echo "::warning title=DeployIQ Indexing Failed::API returned HTTP $HTTP_CODE"
              echo "$BODY"
            else
              echo "$BODY"
            fi
